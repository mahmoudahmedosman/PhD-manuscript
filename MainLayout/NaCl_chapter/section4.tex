...
the minimizer and can lead to a decrease in $\chi_{\text{tot}}^2$, it can prevent the 
...
To address these issues, we developed a sequential training procedure that 
...
\begin{enumerate}
    ...
    estimated for each supernova. Since the recalibration model is linear 
    in these coefficients, this step is computationally inexpensive.

    \item \underline{Initial model guess:} 
    The parameters describing the model surfaces $M_0$, $M_1$, and $CL$ are 
    fitted to obtain an initial model that is sufficiently close to the 
    ...
    model are fitted to provide an initial estimate.
