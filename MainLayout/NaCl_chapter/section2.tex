...
\subsection{The SALT2 parameterization}

%\noindent \textcolor{red}{COMMENT : We might have to define the spectral flux in here, for now im assuming that it will be defined in the section prior to this one}

\subsubsection{The model}

If we write down $\lambda$ and $p$ as the wavelength and phase of the supernova measured in the observer frame, we can define the restframe spectral flux density as :
...
In general, the luminosity distances of supernovae are defined using the emitted flux in the standard $B$ band at the time of maximum emission, observed at $10\mathrm{pc}$ from the supernova : 

\begin{equation}
    \Phi_B = \frac{1}{4\pi(10\mathrm{pc})^2} \int \mathcal{S}(\lambda , p=0) \frac{\lambda}{hc}B(\lambda) \mathrm{d}\lambda
\end{equation}

By imposing the normalization of the model in the restframe $B$ band at phase 0 :
...
The most common way to parameterize $\mathcal{S}$ is by using the SALT parameterization (\cite{Guy_2007_SALT2}). It is defined as the following :
