...
\begin{table}[ht]
    ...
    \end{tabular}
\end{table}
We then use this "synthetic" N-body simulation to generate ZTF-like supernovae using skysurvey and then evaluate the BORG-supernova likelihood (Equation \ref{eqn:borg_pv_L}) 
...
The velocity dispersion is prefered around $200$ km/s. 
...
We start the sampling of initial 
conditions during 1000 steps while keeping the cosmological and supernova model parameters fixed in order to get an initial estimate of the true underlying field. 
We then sample the 
model parameters $\alpha$ and $\mu_A$ alongside the bulk flow $\mathbf{V}$ and the velocity dispersion $\sigma_v$ for another 1000 steps, while continuing to sample the 
