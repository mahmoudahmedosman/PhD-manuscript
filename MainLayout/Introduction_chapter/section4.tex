...
epochs of the Universe. Recent results from the Chicago-Carnegie Hubble Program (CCHP) (\cite{Freedman_2025}) have measured using independent data from the James Webb Space Telescope 
with three independent methods (Tip of the Red Giant Branch(TRGB), J-Region Asymptotic Giant Branch (JAGB) and Cepheids) the $H_0$ value. Their results show that with TRGB and 
...
for peculiar velocities when measuring $H_0$ using the same data as the one used in SH0ES and obtained a smaller value than the one reported by SH0ES of $H_0 = 71.1\pm 1.4 \mathrm{km\,s^{-1}\,Mpc^{-1}}$ inciting the importance of modeling peculiar velocities. 
...
of cosmic shear, galaxy clustering, and galaxy-galaxy lensing (\cite{DES_y6}), indicating a $2.3\sigma$ discrepancy. \\
