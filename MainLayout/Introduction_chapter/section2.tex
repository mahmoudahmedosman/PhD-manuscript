...
although on large scales the Universe looks homogeneous, the smaller scales indicate the existence of an underlying inhomogeneous structure. This can be seen through the formation of galaxy clusters 
and the cosmic web. Understanding how these structures are formed informs us on the strength of gravity at large scales as well as the amount of matter in the Universe.
Since the full derivation of each equation in structure formation is too long to write for this thesis, we recommend reading \cite{Knobel_2013}, \cite{Dodelson_2003} and \cite{Peebles_1980} for a more detailed derivation.\\


\subsection{Newtonian Approach to Structure Formation} \label{sec:newton}
...
\begin{equation}
    \begin{cases}
        \frac{\partial \rho}{\partial t} + \vec{\nabla}. (\rho \vec{v}) = 0 & \text{Continuity\; equation\;} \\
        \frac{\partial \vec{v}}{\partial t} + (\vec{v} . \vec{\nabla}) \vec{v} + \frac{\vec{\nabla} P }{\rho} + \vec{\nabla}\Phi = 0 & \text{Euler\; equation\;} \\
        \nabla^2 \Phi = 4 \pi G \rho & \text{Poisson\; equation\;} \\
        \frac{\partial S}{\partial t } + (\vec{v} . \vec{\nabla}) S = 0& \text{Entropy\; conservation\;}
    \end{cases}
    \label{eqn:hydro_eqn}
\end{equation}
...
require solving the general relativistic Boltzmann equation including all cosmic components and their interactions. This is in itself a difficult problem due to the nonlinear 
coupling between the fluid equations. To overcome that issue, we assume a FLRW Universe containing cold dark matter (treated as a fluid) and perturb the fluid around the 
Hubble flow, meaning we separate each variable as a homogeneous part and an inhomogeneous perturbation. The cold dark matter assumption justifies the use of Newtonian mechanics 
since we are dealing with non-relativistic components. Moreover we assume that the perturbations that arise are very small. We can then write down 
the expansion of the quantities used in the hydrodynamical equation at first order as:
\begin{align*} 
...
\begin{equation}
    \delta q(t, \mathbf{k}) = \int \delta q (t, \mathbf{x}) e^{-i \mathbf{k}\mathbf{x}} \text{d}^3x
\end{equation}

\begin{equation}
    \delta q(t, \mathbf{x}) = \frac{1}{(2\pi)^3} \int \delta q (t, \mathbf{k}) e^{i \mathbf{k}\mathbf{x}} \text{d}^3k
\end{equation}
With $\mathbf{k}$ are the comoving Fourier modes. We get the following hydrodynamical equations at first order:
\begin{equation}
    \boxed{
    \begin{cases}
        \delta \dot{\rho} + 3H\delta \rho + \frac{i\bar{\rho}\mathbf{k}}{a} . \delta\mathbf{v} = 0& \text{Continuity\; equation\;} \\
        \delta \dot{\mathbf{v}} + H \delta \mathbf{v} + \frac{i\mathbf{k}}{a\bar{\rho}}(c_s^2 \delta \rho + \sigma \delta S) + \frac{i \mathbf{k}}{a} \delta \Phi = 0 & \text{Euler\; equation\;} \\
        k^2 \delta \Phi + 4 \pi G a^2 \delta \rho = 0 & \text{Poisson\; equation\;} \\
        \delta \dot{S} = 0& \text{Entropy\; conservation\;}
    \end{cases}
    }
...
What we are interested in is essentially how matter perturbations grow as they will give birth to structure by gravitational attraction. The first thing to note is 
...
models of inflation do not support the fluctuations of entropy. So from now on we will set $\delta S = 0$. We can also assume that the velocity perturbations would follow the overdense regions
without experiencing any vorticity giving us modes that are parallel to the velocity $\mathbf{k} || \delta \mathbf{v}$. 
We call the modes that result from these conditions the adiabatic modes. \\

We will be denoting the overdensity $\delta(\mathbf{k},t)$ as the matter density 
contrast which is written as:
\begin{equation}
    \delta(\mathbf{k}, t) = \frac{\rho(\mathbf{k},t)}{\bar{\rho}(t)} - 1 = 
    \frac{\delta \rho (\mathbf{k}, t)}{\bar{\rho}(t)}
\end{equation}
Therefore, we get the following expression for the time derivative of the 
perturbed density:
\begin{equation}
    \dot{\delta\rho} = \frac{d}{dt}(\bar{\rho}\delta) = \dot{\bar{\rho}}\delta 
    + \bar{\rho}\dot{\delta}
\end{equation}
...
\begin{equation}
    \dot{\bar{\rho}} = -3H\bar{\rho}
\end{equation}
Substituting into the perturbed continuity equation 
(Equation \ref{eqn:newton_hydro_eqn}):
\begin{equation}
    \dot{\delta\rho} + 3H\delta\rho + \frac{i\bar{\rho}\mathbf{k}}{a} \cdot 
    \delta\mathbf{v} = 0 \implies -3H\bar{\rho}\delta + \bar{\rho}\dot{\delta} 
    + 3H\bar{\rho}\delta + \frac{i\bar{\rho}\mathbf{k}}{a} \cdot \delta\mathbf{v} 
    = 0
\end{equation}
where the first and third terms cancel exactly. Dividing by $\bar{\rho}$, the 
...
\end{equation}
Here we obtain a linear second-order differential equation with two solutions to it. 
We denote $\lambda = \frac{2\pi a}{k}$ the physical wavelength of the mode, 
...
place ourselves in the condition where the wavelength is much larger than the Jeans length $\lambda \gg \lambda_J$. 
This approximation is particularly relevant for cold dark matter, which has a negligible sound speed. 
Equation \ref{eqn:linear_dens} is now reduced to:
\begin{equation}
...
\end{equation}
with the + and - denoting growing and decaying modes. These modes highlight the competing forces due to the internal pressure and the gravitational collapse. 
We call the growing term $D_+(t)$ the \textbf{linear growth function}. In the radiation dominated era, the Mészáros 
...
\begin{equation}
    D_- \propto t^{-1}
\end{equation}
This means that we can get growing modes for matter perturbations in the matter dominated era with $D_+$ while $D_-$ goes to 0. More general formulations for $D_+$ can be obtained 
such as the one found in \cite{Carroll_1992}. \\

The growth of perturbations described by $D_+$, called the \textit{growth factor}, can be characterized by the \textit{growth rate} $f$, defined as the logarithmic derivative of the growth factor 
...
\end{equation}
where $\gamma$, also called the growth index, is obtained from the underlying gravitational theory.  
The growth index can be related to the dark energy equation of state as seen in \cite{Linder_2005}:
\begin{equation}
    \gamma \approx 0.55 + (1+w(z))
\end{equation} 
For a flat $\Lambda$CDM cosmology and GR it is $\gamma \simeq 0.55$. Deviations from this value would indicate modifications to GR on cosmological scales. 
...
averages are affected by statistical fluctuations known as sample variance, or \textbf{cosmic variance} when limited by the size of the observable Universe.\\

We denote the 2-point correlation function as:
\begin{equation}
    \xi(\mathbf{x}, \mathbf{x}') = \left\langle \delta(\mathbf{x}) \delta(\mathbf{x}') \right\rangle
\end{equation}
Our field is homogeneous and isotropic therefore the only 
...
then we can decompose it in Fourier modes:
$\delta(\mathbf{k})$:
\begin{equation}
...
\end{equation}
where $\mathcal{P}(k)$ is the \textbf{power spectrum}, which is a real function, and $\delta_D$ is the Dirac delta function. \\
The works from \cite{Harrison_1970}, \cite{Zeldovich_1970_pk} and \cite{Peebles_1970} show that there is a preferred form for the initial primordial power spectrum that takes the form:
\begin{equation}
...
\begin{equation}
    \mathcal{P}_{\text{lin}}(t, k) = A_s k^{n_s}T^2(k)D^2_+(t)
\end{equation} \label{eqn:pk_lin}
with $A_s$ the normalization parameter and $T(k)$ is the transfer function which characterizes how each Fourier mode of the density field evolved from the initial conditions to 
a later time. \\
...
every single quantity, however explanation of how we get there and how it relates to the Newtonian approximation will be present. \\
...
It is defined as $a \text{d}\tau = \text{d}t$. We will also work in natural units setting $c=\hbar =1$. We will denote the FLRW metric as $\bar{g}_{\mu \nu}$ so that 
...
In a relativistic framework, the perturbations are done on the metric itself such that when we choose a metric $g_{\mu \nu}$ the deviations from the homogeneous isotropic 
background are close enough to $\bar{g}_{\mu \nu}$
\footnote{Mathematically, this change in the metric also implies a change 
in the spacetime manifold on which it is defined, meaning that simple operations such as subtracting the two metrics are not strictly well-defined. 
One way to overcome this issue is to introduce an identification between the background and perturbed manifolds, allowing the background 
metric $\bar{g}_{\mu\nu}$ to be treated consistently under coordinate transformations.}. 
We then obtain:
\begin{equation}
    \delta g_{\mu \nu} = g_{\mu \nu}(x) - \bar{g}_{\mu \nu}(x)
\end{equation} 
...
require us to set a gauge transformation. Similarly, the perturbation for a general scalar-like object and a vector-like object are defined as:
\begin{equation}
...
\end{equation}
where $A(\tau, \mathbf{x})$, $B_i(\tau, \mathbf{x})$, $H_L(\tau, \mathbf{x})$ and $H_{ij}(\tau, \mathbf{x})$ are perturbation quantities and $\delta_{ij}$ a Kronecker delta function.\\

\noindent \textbf{\underline{Newtonian gauge}} \\

These perturbation quantities will then intervene in the field equations. Determining them necessitates a gauge transformation. We will be showing the results 
...
\begin{equation}
    B=H=0
\end{equation}
and we will be renaming the other perturbations as:
\begin{equation}
    \Psi = A
\end{equation}
\begin{equation}
...
If we consider scales much smaller than the Hubble horizon $H/k \ll 1$, the first field equation then becomes:
\begin{equation}
...
This also means we can have a physical interpretation for $\Phi$ acting as the Newtonian gravitational potential (called generalized Newtonian potential). The last equation 
...
One can also derive the equations of motion in the Newtonian gauge, however for this thesis we will only be showing the final equations of motions obtained for matter 
...
\end{equation}
which is a reduced version of Equation \ref{eqn:linear_dens} where $c_s=0$.
...
What we see as well is that through inflation we can have a prediction for the initial density fluctuations. 
...
\begin{equation}
    S = \int d^4x \sqrt{-g} \left[ \frac{M^2_{\text{pl}}}{2} R - \frac{1}{2} g^{\mu \nu} \partial_{\mu}\phi \partial_{\nu}\phi - V(\phi) \right]
\end{equation}
where $M_{\text{pl}} $ is the Planck mass defined as $M^2_{\text{pl}} = \left(\frac{1}{8\pi G}\right)^2$. Varying the action with respect to the metric allows us to obtain 
...
\begin{equation}
    H^2 = \frac{1}{M^2_{\text{pl}}}\left( \frac{1}{2} \dot{\phi}^2 + V(\phi) \right)
\end{equation}
\begin{equation}
    \dot{H} = -\frac{1}{2} \frac{\dot{\phi}^2}{3M^2_{\text{pl}}}
\end{equation}

Assuming a slow-rolling field entails that the kinetic term is small compared to the potential term $V$. This assumption yields $a \sim e^{Ht}$, which is a necessary
condition to inflation. Additionally we assume coherent oscillations around a minimum that are damped by $H\dot{\phi}$ as seen in the Klein-Gordon equation. The field is 
then expected to decay into matter and radiation, although the exact physical process behind that is unknown. The 
inflaton field is then treated as a perturbation around a mean field, that these perturbations give rise to perturbation amplitudes that we can identify in the observations we make today. 
Some useful parameters to define for a slow-roll inflation are :
\begin{equation}
...
its canonical conjugate momentum $\hat{\pi}$. In Fourier space, they are expressed as functions of the creation and annihilation operators $a_{\mathbf{k}}$ and $a_{\mathbf{k}}^{\dag}$. 
With these operators we can calculate the expected value of the field as well as the power spectrum as a function of $k$. 
...
\begin{equation}
    \mathcal{P}_{\mathcal{R}}(k) = \frac{H^4}{4\pi^2 \dot{\bar{\phi}}^2}\bigg|_{k=aH} \approx \frac{H^2}{8\pi^2 M_{\text{pl}}^2 \epsilon}\bigg|_{k=aH}
\end{equation}
...
\begin{equation}
    \mathcal{P}_{\text{lin}}(\tau, k) = A_s \left(\frac{k}{k_*}\right)^{n_s-1} T^2(k)\, D_+^2(\tau)
\end{equation}
...
growth of perturbations during matter domination (\cite{Carroll_1992}). The $k^{n_s-1}$ factor reflects the nearly scale-invariant primordial spectrum, with 
