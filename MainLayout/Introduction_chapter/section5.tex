...
When interpreted as an energy content, $\Lambda$ can be identified 
with the vacuum energy predicted by quantum field theory 
(QFT), which arises from the zero-point fluctuations of quantum 
fields. However, identifying the cosmological constant with the vacuum energy gives rise to the \textit{cosmological constant problem} (\cite{Weinberg_1989, Carroll_2001}). 
Depending on the physical origin of the vacuum energy considered, quantum field theory predicts vacuum energy densities between $\sim 10^{40}$ and $\sim 10^{120}$ 
times larger than the observed value, motivating the exploration of alternative dark energy models.
...
The quintessence model is a special case of dynamical dark energy that uses an approach similar to inflation where one assumes the accelerated expansion is driven by a scalar field $\phi$, here called the quintessence field (\cite{Ratra_1988}). 
...
however this is not possible since we know that for an accelerating Universe $w<-\frac{1}{3}$. 
Using a variety of parametric and non-parametric reconstructions, \cite{Lodha_2025} found that the preference for dynamical dark energy is robust to the choice of parametrization. \\ 
...
the chameleon mechanism (\cite{Khoury_2004}) and the best-known $f(R)$ theories do possess it. \\
...
Observations have ruled out the original formulation of the DGP theory (\cite{Fang_2008}), however different formulations such as the normal branch (nDGP,  \cite{Lue_2006}) remain viable. 
\\
