...
Since the discovery of the acceleration of the Universe by \cite{Riess_1998} and \cite{Perlmutter_1999}, the quest to understand the underlying nature of this acceleration has 
seen many theories arise on what it could be. We've seen one way this can be done by via the addition of a cosmological constant that introduces a new type of energy in the 
Einstein equations. Due to the fact that we do not know enough about the nature of this energy it is called "dark energy". In the next subsections we will be discussing some 
of the theoretical models that were proposed to explain this acceleration. \\
...
which is what is done in the $\Lambda$CDM model. We can look at $\Lambda$ in two different lights, either a modification to the geometry of spacetime or as a part of the 
...
One could naturally assume that if the acceleration of the Universe is driven by a fluid that its energy density would change throughout time. The common way to model this 
...
\begin{equation}
    H(z) = H_0 \sqrt{\Omega_m(1+z)^3 + \Omega_r(1+z)^4 + \Omega_k(1+z)^2 + e^{\left( -3w_a\frac{z}{1+z} \right)} \Omega_{\Lambda}(1+z)^{3(1+w_0+w_a)} }
\end{equation}

$\Lambda$CDM then becomes a particular case where $w_0=-1$ and $w_a=0$.
...
however this is not physical since we know that for an accelerating Universe $w<-\frac{1}{3}$. Results from \cite{DESI_DR2_2025} show a preference for a 
...
One of the models that do that are the $f(R)$ models (\cite{Sotiriou_2010}) which replaces the Ricci scalar $R$ by a general function of $R$. The action for an $f(R)$ gravity becomes:
\begin{equation}
...
\end{equation}
If one rewrite Equation \ref{eqn:friedmann_fR} to match what we get in GR $3H^2 = 8\pi G\rho$, we can identify a and effective "curvature fluid" that under certain conditions 
...
the chameleon mechanism and the most well known $f(R)$ theories do possess it. \\
...
By analogy, gravity is expected to be mediated by a spin-2 particle called the graviton.The graviton is predicted to be massless 
...
\begin{equation}
    \Phi = - \frac{GM}{r}e^{-mr}
\end{equation}
where $r$ is the distance to an object, $M$ the mass of the object and $m$ the mass of the graviton. However, the introduction of a massive gravitons produces multiple 
instabilities such as the inability to recover GR when $m$ goes to 0. It also suffers to produce flat FLRW solutions.\\
...
and Massimo Porrati (\cite{Dvali_2000}) where gravity can propagate in a 5-dimensional space time with an additional spatial dimension. We can think of it as our 4D world embedded 
...
\end{equation}
where $M_5$ is the Planck mass $M_{pl}$ in 5D, $\tilde{g}$ and $\tilde{R}$ are the norm of the metric and the Ricci scalar in 5D. The $S_{GH-}$ and $S_{GH+}$ are Gibbons-Hawkings 
terms that serve as boundary terms commonly found in, but not limited to, quantum gravity theories.\\ 
The theory does predict a late time acceleration (\cite{Deffayet_2001}), however it suffers theoretical instabilities and unphysical solutions (\cite{Koyama_2007}). Observations have also ruled the theory out 
