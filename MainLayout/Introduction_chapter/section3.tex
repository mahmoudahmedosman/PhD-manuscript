...
power spectrum $C_\ell$ (where $\ell$ denotes the degree if the spherical harmonic function, also known as the multipole number), 
...
into the large-scale structures we observe today. Secondary anisotropies, arising from effects such as gravitational lensing by galaxy clusters (\cite{Hu_2000}) and inverse Compton 
...
$\Omega_b$, $H_0$, $n_s$, and $A_s$ (\cite{Planck_2018_params}). Figure \ref{fig:pk_planck_angular} shows angular power spectrum comparisons between different CMB 
experiments (such as Planck or WMAP) and the $\Lambda$CDM prediction in a dashed grey line.\\
...
\begin{figure}[ht]
    ...
    around 100 $\text{Mpc}.h^{-1}$ accross all catalogues}
    \label{fig:desi_bao}
\end{figure}
...
\cite{Planck_2018_params}). Recent use of the RSD allowed to constrain the modified gravity using the DESI data (\cite{Adame_2025}). This is represented in Figure \ref{fig:desi_MG} where the parameters constrained 
...
characterised by the parameter $S_8 = \sigma_8(\Omega_m / 0.3)^{0.5}$,  which current surveys constrain to percent-level precision (\cite{KiDS_2021} \cite{DES_Y3_2022} \cite{DES_y6}, \cite{Dalal_2023}).\\
